...
if start_column in df.columns and end_column in df.columns:
    ...
    au_start_idx = all_cols.index('AU01_r')
    file_end_idx = all_cols.index('file_name')

    # 确定要过滤的列范围
    filter_start = min(au_start_idx, file_end_idx)
    filter_end = max(au_start_idx, file_end_idx)
